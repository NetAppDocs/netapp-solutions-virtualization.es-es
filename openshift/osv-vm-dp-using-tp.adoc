...
---
= Protege las máquinas virtuales en Red Hat OpenShift Virtualization usando Trident Protect
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
Los discos persistentes de las VMs de OpenShift Virtualization pueden estar respaldados por almacenamiento ONTAP integrado al Cluster de OpenShift usando link:https://docs.netapp.com/us-en/trident/["Trident CSI"]. En esta sección usamos link:https://docs.netapp.com/us-en/trident/trident-protect/learn-about-trident-protect.html["Trident Protect"] para crear snapshots y backups de las VMs, incluyendo sus volúmenes de datos, en ONTAP Object Storage.

Luego restauramos desde una instantánea o una copia de seguridad cuando sea necesario.

Trident Protect permite realizar instantáneas, copias de seguridad, restauraciones y recuperación ante desastres de aplicaciones y VMs en un clúster OpenShift. Para OpenShift Virtualization VMs, los datos que se pueden proteger con Trident Protect incluyen objetos de recursos de Kubernetes asociados con las VMs, volúmenes de almacenamiento persistente e imágenes internas.
...
== Restauración de un único archivo para una VM

Este método de restauración de un único archivo solo se aplica a volúmenes NAS o basados en archivos que estén montados como un sistema de archivos en el pod cliente, como los volúmenes NFS. No se aplica a volúmenes SAN o de bloques sin formato, incluidos los discos de máquinas virtuales aprovisionados con `volumeMode: Block` como se mostró antes.

Para los volúmenes NAS o basados en archivos compatibles, una forma sencilla de restaurar un archivo específico es hacer que el directorio de instantáneas en el volumen de respaldo sea visible para el cliente. Esto se puede hacer editando la configuración del backend de Trident para incluir la sección defaults y establecer el parámetro  `snapshotDir` en  `true`. Esto permite acceder al directorio  `.snapshot` desde la misma ubicación de montaje que el volumen principal. El directorio  `.snapshot` contiene todas las instantáneas. Puedes cambiar al directorio de instantáneas que necesites, buscar el archivo que quieres restaurar y copiarlo a la ubicación que prefieras.


NOTE: Este método expone datos históricos a cargas de trabajo o usuarios con acceso de montaje y puede incumplir las expectativas de privilegio mínimo o cumplimiento. Por favor, considera el impacto en la seguridad y el funcionamiento, y ten en cuenta que deberías habilitarlo temporalmente y revertirlo después de restaurar un archivo. Además, si estás usando la versión de NFS 4, no puedes ver el directorio `.snapshot` cuando listas archivos con un comando `ls`. Sin embargo, puedes `cd` al directorio `.snapshot`.

.Ejemplo de restauración de un único archivo accediendo al directorio .snapshot y copiando un archivo de una snapshot
[%collapsible]
====
[source, yaml]
----
...
----
image:rh-os-n-use-case-ocpv-tp-dp-sfr-001.png["el pod y el PVC de la máquina virtual"]

image:rh-os-n-use-case-ocpv-tp-dp-sfr-002.png["Nombre del volumen ONTAP para el disco de la máquina virtual"]

image:rh-os-n-use-case-ocpv-tp-dp-sfr-003.png["directorio de instantáneas visible en el cliente"]

image:rh-os-n-use-case-ocpv-tp-dp-sfr-004.png["montar el directorio en la máquina virtual"]

image:rh-os-n-use-case-ocpv-tp-dp-sfr-005.png["perder un archivo en la VM y copiar el archivo del directorio de instantáneas al directorio principal para restaurarlo"]
