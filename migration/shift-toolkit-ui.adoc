...
+
Nota: Para una mejor experiencia, utilice Google Chrome o Internet Explorer.
...
+
Nota: Los administradores deben cambiar sus credenciales durante el inicio de sesión inicial.

+
Nota: Después del cambio de contraseña obligatorio, las credenciales de administrador también se pueden cambiar utilizando la opción “Cambiar contraseña” después de iniciar sesión en la interfaz gráfica de usuario (GUI).
...
Nota: También se pueden agregar sitios utilizando la opción Descubrir.
...
Nota: Si se realizan modificaciones en el sitio de origen, asegúrese de ejecutar la detección para obtener la información más reciente.  Esto se puede hacer haciendo clic en los 3 puntos que aparecen junto al nombre del sitio y luego en “Descubrir sitio”.

Nota: El inventario de máquinas virtuales se actualiza automáticamente cada 24 horas.
...
Nota: El sistema de almacenamiento de origen y destino debe ser el mismo, ya que la conversión del formato del disco se produce a nivel de volumen y dentro del mismo volumen.
...
Nota: Asegúrese de que los Qtrees estén aprovisionados (como se menciona en la sección de requisitos previos) antes de crear los grupos de recursos.
...
+
Nota: Mueva las máquinas virtuales a convertir o migrar a un almacén de datos designado en una ONTAP SVM recién creada antes de la conversión. Esto ayuda a aislar el almacén de datos NFS de producción, y el almacén de datos designado se puede usar para preparar las máquinas virtuales.

+
Nota: Para entornos OpenShift, los VMDK deben reubicarse en sus volúmenes correspondientes para replicar la estructura PVC (Persistent Volume Claim) utilizando el controlador de almacenamiento NAS de ONTAP .  En futuras versiones, se incorporarán mejoras adicionales para aprovechar el controlador de economía de ONTAP NAS.

+
image::shift-toolkit-063.png[Diálogo de la interfaz de usuario Crear nuevo grupo de recursos]

+
Nota: En este contexto, el menú desplegable de almacenamiento de datos solo mostrará los almacenes de datos NFSv3. Los almacenes de datos NFSv4 no se mostrarán.
...
+
Nota: Asegúrese de que la ruta de destino (donde se almacenan las máquinas virtuales convertidas) esté configurada como un qtree al convertir las máquinas virtuales.  Establezca la ruta de destino al qtree apropiado.

+
Nota: Se pueden crear y utilizar varios qtrees para almacenar los discos de la máquina virtual convertida, según corresponda.

+
Nota: Las máquinas virtuales pueden ejecutarse en almacenes de datos distribuidos, y Shit toolkit las detectará automáticamente; sin embargo, se debe asignar un qtree para cada volumen.
...
+
Nota: En caso de necesitar modificar el grupo de recursos para agregar o eliminar máquinas virtuales, utilice esta opción junto al nombre del grupo de recursos y seleccione “Editar grupo de recursos”.
...
+
Nota: Para la migración de prueba, la opción predeterminada es “No configurar la red” y Shift toolkit no realiza la asignación de direcciones IP.  Una vez convertido el disco y comprada la máquina virtual en el hipervisor correspondiente, asigne manualmente los conmutadores de red burbuja para evitar cualquier colisión con la red de producción.
...
+
Nota: Asegúrese de que el qtree esté aprovisionado previamente y que se hayan asignado los permisos necesarios para que la máquina virtual pueda crearse y encenderse.  Nota: En el caso de OpenShift, los PVC se crean utilizando Trident CSI y no es necesario crear previamente los qtrees.
...
+
Nota: Al programar, elija una fecha que sea al menos 30 minutos anterior a la hora actual de la máquina virtual de Shift. Esto es para garantizar que el flujo de trabajo tenga tiempo suficiente para preparar las máquinas virtuales dentro del grupo de recursos.
...
+
Nota: Para una máquina virtual de origen Linux que ejecute CentOS o Red Hat, el kit de herramientas Shift es inteligente y instala automáticamente los controladores Hyper-V necesarios. Estos controladores deben estar presentes en la VM de origen antes de la conversión del disco para garantizar que la VM pueda iniciarse correctamente después de la conversión.

+
Nota: Para obtener información detallada, consultelink:https://access.redhat.com/solutions/3465011["El sistema se queda atascado en Dracut después de migrar una máquina virtual RHEL a Hyper-V."] .
