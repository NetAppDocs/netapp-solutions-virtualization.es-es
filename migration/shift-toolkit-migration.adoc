...
* link:shift-toolkit-migrate-esxi2osvmtv.html["De VMware ESXi a Red Hat OpenShift Virtualization usando Migration Toolkit for Virtualization (MTV)"]
...
Un blueprint que nunca se haya ejecutado puede eliminarse directamente desde la interfaz de usuario. Sin embargo, una vez que se ha activado un blueprint, crea registros del historial de tareas que deben eliminarse antes de poder eliminar el propio blueprint. Usa el script de eliminación de PowerShell para limpiar estos registros de tareas, después podrás eliminar el blueprint y su grupo de recursos asociado.

.Pasos
. Descarga el script de eliminación desde la interfaz de usuario de Shift Toolkit accediendo a *Configuración > Acceso para desarrolladores > Bloque de scripts*.
+
image::shift-toolkit-082.png[Eliminar trabajos según el nombre del blueprint]

. Descomprime la carpeta zip. El paquete contiene dos archivos:
+
** `removeBpJobs.json` - Archivo de configuración
** `removeBpJobs.ps1` - script de PowerShell


. Edita el  `removeBpJobs.json` archivo para especificar:
+
** Nombre de ejecución de esta operación
** Dirección IP de Shift Toolkit
** Credenciales de administrador
** Nombre del blueprint que quieres eliminar
+
...
. Abre PowerShell y ve a la carpeta que contiene los archivos extraídos.
. Ejecuta el script:
+
[listing]
----
...
Después de que el script se complete correctamente, puedes eliminar el blueprint y su grupo de recursos asociado desde la interfaz de usuario de Shift Toolkit.
